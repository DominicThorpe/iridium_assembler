...
impl InstrTokens {
    fn new(label:Option<String>, opcode:String, operand_a:Option<String>, 
        operand_b:Option<String>, operand_c:Option<String>, op_label:Option<String>) -> InstrTokens {
            InstrTokens {
                label: label,
                opcode: opcode,
                operand_a: operand_a,
...
/// Takes the name of a file and iterates through its lines, generating a `Vec<InstrTokens>` from those lines 
/// representing their various components.
///
    ...
    for line_buffer in input_file.lines() {
        ...
        tokens_vec.append(
            &mut match operands.len() {
                0 => vec![InstrTokens::new(label, opcode.to_owned(), None, None, None, None)],
                1 => vec![InstrTokens::new(label, opcode.to_owned(), Some(operands[0].clone()), None, None, None)],
                2 => vec![InstrTokens::new(label, opcode.to_owned(), Some(operands[0].clone()), Some(operands[1].clone()), None, None)],
                4 => vec![InstrTokens::new(label, opcode.to_owned(), Some(operands[0].clone()), Some(operands[1].clone()), Some(operands[2].clone()), Some(operands[3].clone()))],
                3 => { // may or may not contain a label as an operand
                    let mut tokens = vec![InstrTokens::new(label.clone(), opcode.to_owned(), Some(operands[0].clone()), Some(operands[1].clone()), Some(operands[2].clone()), None)];
                    if operands[2].starts_with("@") {
                        tokens = vec![InstrTokens::new(label, opcode.to_owned(), Some(operands[0].clone()), Some(operands[1].clone()), None, Some(operands[2].clone()))]
                    }
...
mod tests {
    ...
    fn test_token_generation() {
        ...
        assert_eq!(tokens[0].operand_c.as_ref().unwrap(), "1");
