                ...
                match &t.op_label {
                    Some(operand) => {
                        if t.opcode == "LOAD" || t.opcode == "STORE" {
                            new_tokens.push(FileTokens::InstrTokens(InstrTokens::new(t.label.clone(), "MOVLI".to_owned(), Some("$ua".to_owned()), None, None, None, Some("u".to_string() + &*operand.clone()))));
                            new_tokens.push(FileTokens::InstrTokens(InstrTokens::new(None, "MOVUI".to_owned(), Some("$ua".to_owned()), None, None, None, Some("u".to_string() + &*operand.clone()))));
...
mod tests {
    ...
    fn test_load_substitution() {
        ...
        assert_instr_token(token, "MOVLI".to_string(), Option::from("$ua".to_string()), None, None, None, Option::from("u@test_1".to_string()));

        token = subbed_tokens[2].try_get_instr_tokens().unwrap();
        assert_instr_token(token, "MOVUI".to_string(), Option::from("$ua".to_string()), None, None, None, Option::from("u@test_1".to_string()));
